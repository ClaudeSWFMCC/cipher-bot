"""
Cipher Bot v5.5 — Trailing Stop + Higher Trade Frequency
...
- BUG FIX: Trade counter now correctly shows open vs closed trades
- TRAILING STOP L1: Price reaches 50% to TP → stop moves to breakeven
- TRAILING STOP L2: Price reaches 75% to TP → stop moves to entry + 1%
- TRAILING STOP L3: Price reaches 90% to TP → stop trails 1.5% below price
- FREQUENCY FIX 1: Removed hidden 'trending' check (was blocking valid signals)
- FREQUENCY FIX 2: RSI threshold raised from 55 to 60 (more entry opportunities)
- FREQUENCY FIX 3: VWAP demoted from hard block to warning only
  - 50MA remains the primary trend filter
  - VWAP False logs a caution but does not prevent entry
- Expected result: 4-8 signals per month vs 1-2 previously
"""
...
# Timeframes
TF_TREND  = 240   # 4-hour candles — trend filter (50MA)
...
# Trailing stop levels (as % of distance from entry to TP)
TRAIL_L1_PCT  = 0.50   # 50% to TP → move stop to breakeven
...
def check_trend(candle_list_4h):
    """
    4-HOUR TREND FILTER
    v5.5: 50MA is the primary hard filter.
    VWAP is now advisory only — logged but does not block entry.
    """
    ...
    # v5.5: Only 50MA required — VWAP is informational
    trend_ok = above_50ma
...
def check_entry(candle_list_1h):
    ...
    1-HOUR ENTRY SIGNAL
    v5.5 changes:
    - RSI threshold raised to 60 (was 55) — more entry opportunities
    - 'trending' check removed — was silently blocking valid signals
    - Now just needs: RSI < 60 + positive momentum above threshold
    """
    ...
    # v5.5 GREEN DOT: RSI below 60 + positive momentum above threshold
    # Removed: trending check (abs(mom10) > 0.1%) — was silently blocking entries
    green_dot = (
        rsi < 60 and
        mom5 > mom_threshold
    )
...
def open_new_trade(entry, signal_candle_1h, trend_ind, entry_ind):
    ...
    # Warn if VWAP is against us
    vwap_warning = "" if trend_ind["vwap_above"] else " ⚠️ VWAP bearish — caution"
    ...
    log.info(
        ...
        f"Above50MA={trend_ind['above_50ma']} | "
...
def update_trailing_stop(price):
    """
    v5.5 Trailing Stop:
    L1 (50% to TP): move stop to breakeven
...
def main():
    global tick_count
    log.info("=" * 60)
    log.info("Cipher Bot v5.5 — Trailing Stop + Higher Trade Frequency")
    log.info(f"Pair: {PAIR} | Mode: {'PAPER' if PAPER else 'LIVE'}")
    log.info(f"Trend Filter: 4h 50MA (primary) | 4h VWAP (advisory)")
    ...
    log.info(f"Trailing: L1@50% breakeven | L2@75% +1% | L3@90% tight trail")
    log.info("=" * 60)
