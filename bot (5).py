"""
Cipher Bot v5.3 — Position Size & Fee Control
...
- FIX 1: Minimum stop distance enforced at 0.5% of entry price
  - Prevents tiny stops from creating massive position sizes
  - At $77,000 BTC the minimum stop is $385 away
- FIX 2: Maximum position size capped at 0.25 BTC per trade
  - Hard cap protects against oversizing regardless of math
  - Keeps fees proportional to actual risk taken
- FIX 3: Fee as % of risk logged on every trade open
  - Makes it easy to spot if fees are eating too much of the risk budget
- Result: Fees should now be ~5-10% of risk, not 200%
"""
...
# v5.3 Position size controls
MIN_STOP_PCT  = 0.015   # Minimum stop distance = 1.5% of entry price (suits BTC volatility)
...
def open_new_trade(entry, signal_candle_1h, trend_ind, entry_ind):
    ...
    # v5.3 FIX 1: Enforce minimum stop distance of 0.5% below entry
    ...
    # v5.3 FIX 2: Cap position size at MAX_BTC_SIZE
...
def main():
    global tick_count
    log.info("=" * 60)
    log.info("Cipher Bot v5.3 — Position Size & Fee Control")
