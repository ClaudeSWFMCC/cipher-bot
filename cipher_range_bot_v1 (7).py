"""
Cipher Range Bot v1.2 — Ranging Market Companion Bot
...
MA_PERIOD        = 20     # 20-period moving average — matches main trend bot
...
# Re-anchor settings (v1.2) — when price lives outside the range, rebuild it fresh
REANCHOR_LOOKBACK   = 5      # shorter window (~20h) to re-anchor a fresh range fast
REANCHOR_OUT_PCT    = 0.005  # price >0.5% beyond a range edge = "living outside"
RANGE_MIN_WIDTH_PCT = 0.01   # re-anchored range must be ≥1% wide or it's noise
...
# Fee model — Kraken Pro base-tier TAKER rate (0.40%/side as of Aug 2026).
# Previously hardcoded at 0.001 (0.10%/side), which understated real trading
# costs by ~4x on every trade. Override via env var if a different fee tier
# genuinely applies (e.g. maker orders, or a higher-volume tier).
FEE_RATE = float(os.environ.get("FEE_RATE", "0.004"))
...
# ── Range Detection ───────────────────────────────────────────────────────────
def _build_range(candles, lookback, price, source):
    """Helper — build a range dict from the last `lookback` candles."""
    recent = candles[-lookback:]
    range_high = max(c["high"] for c in recent)
    range_low  = min(c["low"]  for c in recent)
    range_mid  = (range_high + range_low) / 2
    range_width_pct = (range_high - range_low) / range_mid if range_mid else 0
    atr = compute_atr(candles, period=10)
    atr_pct = atr / range_mid if range_mid else 0
    is_ranging = RANGE_MIN_WIDTH_PCT <= range_width_pct <= RANGE_WIDTH_PCT
    return {
...
def detect_range(candle_list_4h, price=None):
    """
    Identifies if BTC is in a ranging market.

    v1.2 — Aggressive re-anchor (Option A):
    The standard range uses a 10-candle (~40h) window. The problem seen in v1.1:
    when price breaks down, the old highs stay INSIDE that 40h window for hours,
    so the range stays wide and stale while price is pinned to one edge — the
    short zone ends up thousands of dollars away from price and never triggers.

    Fix: compare a short recent window (~20h) against the standard range. If the
    recent window has abandoned a meaningful part of the standard range — i.e.
    recent price action is living entirely in the top or bottom portion of the
    standard range — the standard range is stale. Rebuild it from the short
    window so the zones track where price actually is now.

    A re-anchored range must still be ≥ RANGE_MIN_WIDTH_PCT wide to count as a
    real range — otherwise it's just price pausing mid-move (noise), not a range.
    The trend filter in check_range_entry still gates direction either way.
    """
    if len(candle_list_4h) < RANGE_LOOKBACK + 5:
        return None

    # Standard range from the full lookback window
    rng = _build_range(candle_list_4h, RANGE_LOOKBACK, price, source="standard")

    # Recent short window — where price has actually been living lately
    recent = candle_list_4h[-REANCHOR_LOOKBACK:]
    recent_high = max(c["high"] for c in recent)
    recent_low  = min(c["low"]  for c in recent)

    std_high = rng["range_high"]
    std_low  = rng["range_low"]
    std_span = std_high - std_low

    if std_span > 0:
        # How far into the standard range does the recent window sit?
        # If recent action occupies only the bottom (or top) portion and has
        # abandoned the rest, the standard range is stale.
        recent_top_frac = (recent_high - std_low) / std_span   # 1.0 = touches std high
        recent_bot_frac = (recent_low  - std_low) / std_span   # 0.0 = touches std low

        abandoned_top = recent_top_frac < 0.65   # recent high never gets near std top
        abandoned_bot = recent_bot_frac > 0.35   # recent low never gets near std bottom

        if abandoned_top or abandoned_bot:
            fresh = _build_range(candle_list_4h, REANCHOR_LOOKBACK, price, source="reanchored")
            return fresh
...
def check_trade_exit(price, range_info):
    ...
    direction = open_trade["direction"]
    fee_out   = price * open_trade["size"] * FEE_RATE
...
def print_status(price, range_info, entry_ind):
    ...
    if range_info:
        tag = " ⟳REANCHORED" if range_info.get("source") == "reanchored" else ""
        range_status = (f"RANGING ${range_info['range_low']:,.2f}—${range_info['range_high']:,.2f} "
                       f"({range_info['range_width_pct']}%){tag}"
                       if range_info["is_ranging"] else
                       f"TRENDING (range {range_info['range_width_pct']}%, need {RANGE_MIN_WIDTH_PCT*100}–{RANGE_WIDTH_PCT*100}%){tag}")
    else:
...
def main():
    global tick_count
    log.info("=" * 60)
    log.info("Cipher Range Bot v1.2 — Ranging Market Companion")
    ...
    while True:
        try:
            ...
            if (price
                ...
                range_info  = detect_range(list(candles_4h), price)
