"""
Cipher Bot v5.4 — Relaxed Entry Conditions
...
- FIX 1: Phantom fee bug fixed — total_fees initialized correctly
- CHANGE 1: RSI threshold raised from 45 to 55 (catches more of recovery phase)
- CHANGE 2: Momentum flip requirement removed (prev_mom5 < 0 dropped)
  - Now only requires momentum to be positive and above threshold
  - Old requirement was too strict — missed valid entries waiting for exact flip candle
- CHANGE 3: VWAP switched back to 4h (1h VWAP flipped too fast, blocked good entries)
- Result: Bot should now trade more frequently while 50MA still blocks downtrends
"""
...
losses              = 0
total_pnl           = 0.0
total_fees          = 0.0   # FIX: initialized to 0.0, never pre-populated
...
def check_trend(candle_list_4h):
    """
    4-HOUR TREND FILTER
    Price must be above 50MA and 4h VWAP — confirms uptrend.
    v5.4: Back to 4h VWAP (1h was too noisy, blocked valid entries).
    """
...
def check_entry(candle_list_1h):
    ...
    1-HOUR ENTRY SIGNAL
    v5.4 changes:
    - RSI threshold raised to 55 (was 45) — catches more of the recovery phase
    - Momentum flip requirement removed — just needs positive momentum above threshold
    - Simpler and fires more often while trend filter still blocks downtrends
    """
    ...
    # v5.4 GREEN DOT: RSI below 55 + positive momentum above threshold
    # Removed: prev_mom5 < 0 (flip requirement was too strict)
    green_dot = (
        rsi < 55 and
    ...
    # BLUE TRIANGLE: overbought exit warning (unchanged)
    blue_tri = (
        rsi > 70 and
        mom5 < -mom_threshold and
        trending
...
def open_new_trade(entry, signal_candle_1h, trend_ind, entry_ind):
    ...
    # Enforce minimum stop distance of 1.5% below entry
    raw_stop  = signal_candle_1h["low"] * 0.999
    min_stop  = entry * (1 - MIN_STOP_PCT)
    stop      = min(raw_stop, min_stop)   # whichever is lower = wider stop
    ...
    log.info(
        ...
        f"TP: ${tp:,.2f} | Size: {size:.6f} BTC | Fee-in: ${fee_in:.2f}"
    )
    log.info(
    ...
    log.info(
        f"  Trend (4h): 50MA=${trend_ind['ma50']:,.2f} | "
        f"Above50MA={trend_ind['above_50ma']} | VWAP4h=${trend_ind['vwap_4h']:,.2f}"
    )
    log.info(
...
def check_trade_exit(price):
    ...
    if price <= open_trade["stop"]:
        ...
        log.info(f"{mode} STOP HIT ❌ | Exit: ${price:,.2f} | Net P&L: ${pnl:,.2f} | Fees: ${total_fee:.2f}")
        open_trade = None
        ...
        total_pnl  += pnl
        total_fees += total_fee
        log.info(f"{mode} TP HIT ✅ | Exit: ${price:,.2f} | Net P&L: ${pnl:,.2f} | Fees: ${total_fee:.2f}")
        open_trade = None
...
def print_status(price, trend_ind, entry_ind):
    ...
    log.info(
        ...
        f"Oversold(<55): {entry_ind.get('oversold', False)} | "
...
def main():
    global tick_count
    log.info("=" * 60)
    log.info("Cipher Bot v5.4 — Relaxed Entry Conditions")
    log.info(f"Pair: {PAIR} | Mode: {'PAPER' if PAPER else 'LIVE'}")
    log.info(f"Trend Filter: 4h 50MA + 4h VWAP")
    log.info(f"Entry Signal: 1h RSI <55 + Positive Momentum (no flip required)")
    log.info(f"Capital: ${CAPITAL:,.2f} | Risk: {RISK_PCT}% | TP: {TP_MULTI}x")
    log.info(f"Min Stop: {MIN_STOP_PCT*100:.1f}% of price | Max Size: {MAX_BTC_SIZE} BTC")
    log.info("=" * 60)
